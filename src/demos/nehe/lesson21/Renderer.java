...
class Renderer implements GLEventListener {
	...
	private void update() {
		...
		if ((hourglass.fx == 2) && (hourglass.fy > 500 + (500 * level))) // Is The hourglass fx Variable Equal To 2 And
																			// The fy
	...
	public void drawGLScene(GL gl) {
		...
		if (hourglass.fx == 1) { // If fx=1 Draw The Hourglass
			gl.glLoadIdentity(); // Reset The Modelview Matrix
			gl.glTranslatef(20.0f + (hourglass.x * 60), 70.0f + (hourglass.y * 40), 0.0f); // Move To The Fine Hourglass
																							// Position
			gl.glRotatef(hourglass.spin, 0.0f, 0.0f, 1.0f); // Rotate Clockwise
			gl.glColor3f(random.nextFloat(), random.nextFloat(), random.nextFloat()); // Set Hourglass Color To Random
																						// Color
