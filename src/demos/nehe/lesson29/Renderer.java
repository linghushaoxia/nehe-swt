...
class Renderer implements GLEventListener {
	...
	void blit(TextureImage src, TextureImage dst, int src_xstart, int src_ystart, int src_width, int src_height, int dst_xstart, int dst_ystart, boolean blend, int alpha) {
		int i, j, k;
			...
			for (j = 0; j < src_width; j++) // Width Loop
			{
				for (k = 0; k < src.format; k++, d++, s++) // "n" Bytes At A Time
				{
					if (blend) // If Blending Is On
						dst.data.put(d, (byte) (((src.data.get(s) * alpha) + (dst.data.get(d) * (255 - alpha))) >> 8)); // Multiply
																														// Src
																														// Data*alpha
																														// Add
																														// Dst
	...
	public void init(GLAutoDrawable glDrawable) {
		...
		// Image To Blend In, Original Image, Src Start X & Y, Src Width & Height, Dst Location X & Y, Blend Flag, Alpha
		// Value
