...
class Renderer implements GLEventListener {
	...
	private int createBlurTexture(GL gl) { // Create An Empty Texture
		ByteBuffer data = BufferUtil.newByteBuffer(TEXTURE_SIZE * TEXTURE_SIZE); // Create Storage Space For Texture
																					// Data (128x128x4)
		...
		gl.glTexImage2D(GL.GL_TEXTURE_2D, 0, GL.GL_LUMINANCE, TEXTURE_SIZE, TEXTURE_SIZE, 0, GL.GL_LUMINANCE, GL.GL_UNSIGNED_BYTE, data); // Build
																																			// Texture
																																			// Using
																																			// Information
																																			// In
	...
	private void calcNormal(float[][] v/* [3][3] */, float[] out/* [3] */) { // Calculates Normal For A Quad Using 3
																				// Points
	...
	public void init(GLAutoDrawable drawable) {
		...
		float[] global_ambient = new float[] { 0.2f, 0.2f, 0.2f, 1.0f }; // Set Ambient Lighting To Fairly Dark Light
																			// (No Color)
	...
	private int createFrameBufferObject(GL gl) {
		...
		gl.glTexImage2D( // Build Texture Using Information In data
				GL.GL_TEXTURE_2D, 0, GL.GL_RGBA, TEXTURE_SIZE, TEXTURE_SIZE, 0, GL.GL_RGBA, GL.GL_UNSIGNED_BYTE, BufferUtil.newByteBuffer(TEXTURE_SIZE * TEXTURE_SIZE * 4));
		gl.glTexParameteri(GL.GL_TEXTURE_2D, GL.GL_TEXTURE_MIN_FILTER, GL.GL_LINEAR);
		gl.glTexParameteri(GL.GL_TEXTURE_2D, GL.GL_TEXTURE_MAG_FILTER, GL.GL_LINEAR);
