...
class Object3D {
	...
	public void draw(GL gl) {
		...
		for (int i = 0; i < nPlanes; i++) {
			for (int j = 0; j < 3; j++) {
				gl.glNormal3f(planes[i].normals[j].x, planes[i].normals[j].y, planes[i].normals[j].z);
				gl.glVertex3f(points[planes[i].p[j]].x, points[planes[i].p[j]].y, points[planes[i].p[j]].z);
			}
		}
		gl.glEnd();
	}
	...
	public void castShadow(GL gl, float[] lp) {
		...
		for (int i = 0; i < nPlanes; i++) {
			...
			float side = planes[i].PlaneEq.a * lp[0] + planes[i].PlaneEq.b * lp[1] + planes[i].PlaneEq.c * lp[2] + planes[i].PlaneEq.d * lp[3];
			if (side > 0)
			...
			if (planes[i].visible)
				for (int j = 0; j < 3; j++) {
					...
					if ((k == 0) || (!planes[k - 1].visible)) {
						...
						gl.glVertex3f(points[p1].x, points[p1].y, points[p1].z);
						gl.glVertex3f(points[p1].x + v1.x, points[p1].y + v1.y, points[p1].z + v1.z);

						gl.glVertex3f(points[p2].x, points[p2].y, points[p2].z);
						gl.glVertex3f(points[p2].x + v2.x, points[p2].y + v2.y, points[p2].z + v2.z);
						gl.glEnd();
					}
				}
		}
