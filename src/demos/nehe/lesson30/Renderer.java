...
class Renderer implements GLEventListener {
	...
	public void init(GLAutoDrawable drawable) {
		...
		try {
			audioSample = new AudioSample(ResourceRetriever.getResourceAsStream("demos/data/samples/explode.wav"));
		} catch (IOException e) {
			e.printStackTrace();
	...
	private void loadGLTextures(GL gl) {
		...
		gl.glTexParameteri(GL.GL_TEXTURE_2D, GL.GL_TEXTURE_MAG_FILTER, GL.GL_LINEAR); /*
																					 * scale linearly when image bigger
																					 * than texture
																					 */
		gl.glTexParameteri(GL.GL_TEXTURE_2D, GL.GL_TEXTURE_MIN_FILTER, GL.GL_LINEAR); /*
																					 * scale linearly when image smalled
																					 * than texture
																					 */
		...
		/*
		 * 2d texture, level of detail 0 (normal), 3 components (red, green, blue), x size from image, y size from
		 * image,
		 */
	...
	public void display(GLAutoDrawable drawable) {
		...
		if (cameraAttachedToBall)
			glu.gluLookAt(arrayPos[0].x + 250, arrayPos[0].y + 250, arrayPos[0].z, arrayPos[0].x + arrayVel[0].x, arrayPos[0].y + arrayVel[0].y, arrayPos[0].z + arrayVel[0].z, 0, 1, 0);
		else
			glu.gluLookAt(cameraPosition.x, cameraPosition.y, cameraPosition.z, cameraPosition.x + dir.x, cameraPosition.y + dir.y, cameraPosition.z + dir.z, 0, 1.0, 0.0);
		...
		for (i = 0; i < 20; i++) {
			if (explosions[i].alpha >= 0) {
				...
				gl.glTranslatef((float) explosions[i].position.x / explosions[i].scale, (float) explosions[i].position.y / explosions[i].scale, (float) explosions[i].position.z / explosions[i].scale);
				gl.glCallList(dlist);
				gl.glPopMatrix();
			}
		}
	...
	/**
	 * Main loop of the simulation. Moves, finds the collisions and responses of the objects in the current time step.
	 */
	private void idle() {
