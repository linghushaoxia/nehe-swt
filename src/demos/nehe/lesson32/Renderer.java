...
class Renderer implements GLEventListener {
	...
	private Rectangle2D.Float[] size = { new Rectangle2D.Float(0, 0, 1.0f, 1.0f), new Rectangle2D.Float(0, 0, 1.0f, 1.0f), new Rectangle2D.Float(0, 0, 1.0f, 1.0f),
			new Rectangle2D.Float(0, 0, 0.5f, 1.0f), new Rectangle2D.Float(0, 0, 0.75f, 1.5f) };
	...
	public void init(GLAutoDrawable drawable) {
		...
		targetTypes = new TargetType[] { TargetType.FACE, TargetType.BUCKET, TargetType.TARGET, TargetType.COKE, TargetType.VASE };
	 ...
	 * Note: I think the key handling should be preformed inside the simulation but keeping it here to stay true the
	 * original c++ code
	 * 
	 * @param milliseconds
	...
	public void createNewTarget(int num) {
		...
		Target element = new Target(targetType, level, num, textures[targetTypeIndex], textures[5], size[targetTypeIndex].width, size[targetTypeIndex].height);
