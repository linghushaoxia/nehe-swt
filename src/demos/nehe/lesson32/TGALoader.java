...
public class TGALoader {
	...
	private static TextureImage loadTGA(GL gl, ReadableByteChannel file) throws IOException {
		...
		int width = unsignedByteToInt(header.get(1)) * 256 + unsignedByteToInt(header.get(0)); // Determine The TGA
																								// Width(highbyte*256+lowbyte)
		int height = unsignedByteToInt(header.get(3)) * 256 + unsignedByteToInt(header.get(2)); // Determine The TGA
		...
		return new TextureImage(textureID, width, height, bytesPerPixel, imageData);
	}
