...
class Renderer implements GLEventListener {
	...
	private Object3D loadObject(InputStream inputStream) throws IOException { // Loads Object From File (name)
		...
		int nbVertices = Integer.parseInt(tokenizer.nextToken()); // Scans Text For "Vertices: ". Number After Is Stored
																	// In ver
	...
	void calculate(int i, Vertex a) { // Calculates Movement Of Points During Morphing
		a.x = (sour.points[i].x - dest.points[i].x) / STEPS; // a.x Value Equals Source x - Destination x Divided By
																// Steps
		a.y = (sour.points[i].y - dest.points[i].y) / STEPS; // a.y Value Equals Source y - Destination y Divided By
																// Steps
		a.z = (sour.points[i].z - dest.points[i].z) / STEPS; // a.z Value Equals Source z - Destination z Divided By
																// Steps
	...
	public void init(GLAutoDrawable drawable) {
		...
		try {
			objects[0] = loadObject(ResourceRetriever.getResourceAsStream("demos/data/models/sphere.txt")); // Load The
																											// First
																											// Object
																											// Into
																											// morph1
																											// From File
																											// sphere.txt
			objects[1] = loadObject(ResourceRetriever.getResourceAsStream("demos/data/models/torus.txt")); // Load The
																											// Second
																											// Object
																											// Into
																											// morph2
																											// From File
																											// torus.txt
			objects[2] = loadObject(ResourceRetriever.getResourceAsStream("demos/data/models/tube.txt")); // Load The
																											// Third
																											// Object
																											// Into
																											// morph3
																											// From File
																											// tube.txt
			helper = loadObject(ResourceRetriever.getResourceAsStream("demos/data/models/sphere.txt"));// Load
																										// sphere.txt
																										// Object Into
																										// Helper (Used
																										// As Starting
		...
		for (int i = 0; i < 486; i++) // Loop Through All 468 Vertices
		{
			...
			objects[3].points[i].x = (float) (Math.random() - 0.5) * 7; // morph4 x Point Becomes A Random Float Value
																		// From -7 to 7
			objects[3].points[i].y = (float) (Math.random() - 0.5) * 7; // morph4 y Point Becomes A Random Float Value
																		// From -7 to 7
			objects[3].points[i].z = (float) (Math.random() - 0.5) * 7; // morph4 z Point Becomes A Random Float Value
																		// From -7 to 7
		}
