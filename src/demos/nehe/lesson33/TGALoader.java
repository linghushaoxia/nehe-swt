...
class TGALoader {
	...
	private static void loadUncompressedTGA(Texture texture, ReadableByteChannel in) throws IOException // Load an
																										// uncompressed
																										// TGA (note,
																										// much of this
																										// code is based
	...
	{ // TGA Loading code nehe.gamedev.net)
		...
		texture.width = (unsignedByteToInt(tga.header.get(1)) << 8) + unsignedByteToInt(tga.header.get(0)); // Determine
																											// The TGA
																											// width
																											// (highbyte*256+lowbyte)
		texture.height = (unsignedByteToInt(tga.header.get(3)) << 8) + unsignedByteToInt(tga.header.get(2)); // Determine
																												// The
																												// TGA
																												// height
																												// (highbyte*256+lowbyte)
		...
		if ((texture.width <= 0) || (texture.height <= 0) || ((texture.bpp != 24) && (texture.bpp != 32))) // Make sure
																											// all
																											// information
																											// is valid
		{
		...
		tga.imageSize = (tga.bytesPerPixel * tga.width * tga.height); // Compute the total amout ofmemory needed to
																		// store data
	...
	private static void loadCompressedTGA(Texture texture, ReadableByteChannel fTGA) throws IOException // Load
																										// COMPRESSED
																										// TGAs
	{
		TGA tga = new TGA();
		...
		do {
			...
			if (chunkheader < 128) // If the ehader is < 128, it means the that is the number of RAW color packets minus
									// 1
			{ // that follow the header
				...
				for (short counter = 0; counter < chunkheader; counter++) // Read RAW color values
				{
					...
					// write to memory
					texture.imageData.put(currentbyte, colorbuffer.get(2)); // Flip R and B vcolor values around in the
																			// process
