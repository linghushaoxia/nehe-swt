...
class Renderer implements GLEventListener {
	...
	private void reset() // Reset The Maze, Colors, Start Point, Etc
	{
		...
		{
			r[loop] = (byte) ((int) (Math.random() * Integer.MAX_VALUE) % 128 + 128); // Pick A Random Red Color
																						// (Bright)
			g[loop] = (byte) ((int) (Math.random() * Integer.MAX_VALUE) % 128 + 128); // Pick A Random Green Color
																						// (Bright)
			b[loop] = (byte) ((int) (Math.random() * Integer.MAX_VALUE) % 128 + 128); // Pick A Random Blue Color
																						// (Bright)
		}
	...
	private void update(float milliseconds) // Perform Motion Updates Here
	{
		...
		if ((mx > (MAZE_WIDTH - 4) || (tex_data.get((((mx + 2) + (MAZE_WIDTH * my)) * 3)) != 0)) && (mx < 2 || (tex_data.get((((mx - 2) + (MAZE_WIDTH * my)) * 3)) != 0))
				&& (my > (MAZE_HEIGHT - 4) || (tex_data.get(((mx + (MAZE_WIDTH * (my + 2))) * 3)) != 0)) && (my < 2 || (tex_data.get(((mx + (MAZE_WIDTH * (my - 2))) * 3)) != 0))) {
		...
		{
			if (tex_data.get((((mx + 2) + (MAZE_WIDTH * my)) * 3)) == 0) // And If The Room To The Right Has Not Already
																			// Been Visited
			{
		...
		{
			if (tex_data.get(((mx + (MAZE_WIDTH * (my + 2))) * 3)) == 0) // And If The Room Below Has Not Already Been
																			// Visited
			{
		...
		{
			if (tex_data.get((((mx - 2) + (MAZE_WIDTH * my)) * 3)) == 0) // And If The Room To The Left Has Not Already
																			// Been Visited
			{
				updateTex(mx - 1, my); // Update The Texture To Show Path Cut Out Between Rooms
		...
		{
			if (tex_data.get(((mx + (MAZE_WIDTH * (my - 2))) * 3)) == 0) // And If The Room Above Has Not Already Been
																			// Visited
			{
				updateTex(mx, my - 1); // Update The Texture To Show Path Cut Out Between Rooms
	...
	public void display(GLAutoDrawable drawable) {
		...
		// Update Our Texture... This Is The Key To The Programs Speed... Much Faster Than Rebuilding The Texture Each
		// Time
		...
		for (int loop = 0; loop < 4; loop++) // Loop To Draw Our 4 Views
		{
			...
			if (loop == 3) // Are We Drawing The Fourth Image? (3D Texture Mapped Cylinder... Perspective)
			{
				...
				gl.glEnable(GL.GL_LIGHTING); // Enable Lighting
				gl.glTranslatef(0.0f, 0.0f, -2.0f); // Translate -2 On The Z-Axis (To Rotate Cylinder Around The Center,
													// Not An End)
