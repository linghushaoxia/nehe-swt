...
class Renderer implements GLEventListener {
	/**
	 * ConstantVelocity is an object from Physics1.h. It is a container for simulating masses. Specifically, it creates
	 * a mass and sets its velocity as (1, 0, 0) so that the mass moves with 1.0f meters / second in the x direction.
	 */
	private ConstantVelocity constantVelocity = new ConstantVelocity();

	/**
	 * MotionUnderGravitation is an object from Physics1.h. It is a container for simulating masses. This object applies
	 * gravitation to all masses it contains. This gravitation is set by the constructor which is (0.0f, -9.81f, 0.0f)
	 * for now (see below). This means a gravitational acceleration of 9.81 meter per (second * second) in the negative
	 * y direction. MotionUnderGravitation creates one mass by default and sets its position to (-10, 0, 0) and its
	 * velocity to (10, 15, 0)
	 */
	private MotionUnderGravitation motionUnderGravitation = new MotionUnderGravitation(new Vector3D(0.0f, -9.81f, 0.0f));

	/**
	 * MassConnectedWithSpring is an object from Physics1.h. It is a container for simulating masses. This object has a
	 * member called connectionPos, which is the connection position of the spring it simulates. All masses in this
	 * container are pulled towards the connectionPos by a spring with a constant of stiffness. This constant is set by
	 * the constructor and for now it is 2.0 (see below).
	 */
	private MassConnectedWithSpring massConnectedWithSpring = new MassConnectedWithSpring(2.0f);

	private float slowMotionRatio = 10.0f; // slowMotionRatio Is A Value To Slow Down The Simulation, Relative To Real
											// World Time
	private float timeElapsed = 0; // Elapsed Time In The Simulation (Not Equal To Real World's Time Unless
	...
	public void display(GLAutoDrawable drawable) {
		...
		// Draw All Masses In motionUnderGravitation Simulation (Actually There Is Only One Mass In This Example Of
		// Code)
		...
		// Draw All Masses In massConnectedWithSpring Simulation (Actually There Is Only One Mass In This Example Of
		// Code)
		...
		glPrint(gl, -5.0f, 14, 0, FONT, "Time elapsed (seconds): " + numberFormat.format(timeElapsed)); // Print
																										// timeElapsed
		glPrint(gl, -5.0f, 13, 0, FONT, "Slow motion ratio: " + numberFormat.format(slowMotionRatio)); // Print
																										// slowMotionRatio
	...
	private void update(long milliseconds) // Perform Motion Updates Here
	{
		...
		int numOfIterations = (int) (dt / maxPossible_dt) + 1; // Calculate Number Of Iterations To Be Made At This
																// Update Depending On maxPossible_dt And dt
