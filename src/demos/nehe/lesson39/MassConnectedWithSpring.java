...
class MassConnectedWithSpring extends Simulation {
	...
	public MassConnectedWithSpring(float springConstant) // Constructor firstly constructs its super class with 1 mass
															// and 1 kg
	{
		...
		mass.pos = new Vector3D(connectionPos).add(new Vector3D(10.0f, 0.0f, 0.0f)); // set the position of the mass 10
																						// meters to the right side of
																						// the connectionPos
	...
	{
		Iterator masses = getMasses(); // we will apply force to all masses (actually we have 1 mass, but we can extend
										// it in the future)
		while (masses.hasNext()) {
			Mass mass = (Mass) masses.next();
			Vector3D springVector = new Vector3D(mass.pos).sub(connectionPos); // find a vector from the position of the
																				// mass to the connectionPos
			mass.applyForce(springVector.inverse().scale(springConstant)); // apply the force according to the famous
