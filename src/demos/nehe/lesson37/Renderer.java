...
class Renderer implements GLEventListener {
	...
	public void display(GLAutoDrawable drawable) {
		...
		Vector TmpVector = new Vector(), TmpNormal = new Vector(); // Temporary VECTOR Structures ( NEW )
		...
		for (int i = 0; i < polyNum; i++)
			...
			for (int j = 0; j < 3; j++) { // Loop Through Each Vertex ( NEW )
				...
				gl.glVertex3f(polyData[i].Verts[j].Pos.X, polyData[i].Verts[j].Pos.Y, polyData[i].Verts[j].Pos.Z); // Send
																													// The
																													// Vertex
																													// Position
																													// (
