...
/**
 * I don't mind if you use this class in your own code. All I ask is that you give me credit for it if you do. And plug
 * NeHe while your at it! :P Thanks go to David Steere, Cameron Tidwell, Bert Sammons, and Brannon Martindale for
 * helping me test all the code! Enjoy.
 * 
 * @author Vic Hollis
...
class Camera {
	...
	private void updateFrustum(GL gl) {
		...
		float clip[] = new float[16], proj[] = new float[16], modl[] = new float[16], t;
	...
	public void updateFrustumFaster(GL gl) {
		...
		/*
		 * Combine the two matrices (multiply projection by modelview) but keep in mind this function will only work if
		 * you do NOT rotate or translate your projection matrix
		 */
	...
	public void renderLensFlare(GL gl) {
		...
		if (sphereInFrustum(m_LightSourcePos, 1.0f)) {
			...
			vLightSourceToCamera.sub(m_Position, m_LightSourcePos); // Lets compute the vector that points to the camera
																	// from
			// the light source.
			...
			vLightSourceToIntersect.sub(ptIntersect, m_LightSourcePos); // Lets compute the vector that points to the
																		// Intersect
			...
			pt.scaleAdd(Length * .2755f, vLightSourceToIntersect, m_LightSourcePos);// Lets compute a point that is
																					// 55.1%
			// away from the light source in the
			// direction of the intersection point.
			renderGlow(gl, 0.9f, 0.9f, 0.2f, 0.5f, pt, 0.8f); // Render the small Glow

			pt.scaleAdd(Length * .4775f, vLightSourceToIntersect, m_LightSourcePos);// Lets compute a point that is
																					// 95.5%
			// away from the light source in the
			// direction of the intersection point.
			...
			pt.scaleAdd(Length * .7825f, vLightSourceToIntersect, m_LightSourcePos);// Lets compute a point that is
																					// 156.5%
			// away from the light source in the
			// direction of the intersection point.
