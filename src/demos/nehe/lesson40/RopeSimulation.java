 ...
 * <p>
 * Quote from Erkin Tunca's code...<br/>
 * class RopeSimulation is derived from class Simulation (see Physics1.h). It simulates a rope with point-like particles
 * binded with springs. The springs have inner friction and normal length. One tip of the rope is stabilized at a point
 * in space called "Vector3D ropeConnectionPos". This point can be moved externally by a method
 * "void setRopeConnectionVel(Vector3D ropeConnectionVel)". RopeSimulation creates air friction and a planer surface (or
 * ground) with a normal in +y direction. RopeSimulation implements the force applied by this surface. In the code, the
 * surface is refered as "ground".
 * </p>
 * Date: 26 Oct 2007
 * 
...
public class RopeSimulation extends Simulation {
	...
	private final Vector3D ropeConnectionVel; // A Variable To Move The ropeConnectionPos (By This, We Ccan Swing The
												// Rope)
	 ...
	 * @param springFrictionConstant
	 *            Inner Friction Constant Of Spring
	 * @param gravitation
	 *            Gravitational Acceleration
	 * @param airFrictionConstant
	 *            Air Friction Constant
	 * @param groundRepulsionConstant
	 *            Ground Repulsion Constant
	 * @param groundFrictionConstant
	 *            Ground Friction Constant
	 * @param groundAbsorptionConstant
	 *            Ground Absorption Constant
	 * @param groundHeight
	 *            Height Of The Ground (Y Position)
	 */
	public RopeSimulation(int numOfMasses, // 1. The Number Of Masses
		...
		for (int a = 0; a < numOfMasses; ++a) { // To Set The Initial Positions Of Masses Loop With For(;;)
			masses[a].pos.x = a * springLength; // Set X-Position Of masses[a] With springLength Distance To Its
												// Neighbor
	...
	public void simulate(float dt) { // simulate(float dt) Is Overriden Because We Want To Simulate the Motion Of The
										// ropeConnectionPos
