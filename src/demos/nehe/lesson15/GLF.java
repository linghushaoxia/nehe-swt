 ...
 * 
 * Author: Roman Podobedov Email: romka@ut.ee WEB: http://romka.demonews.com Release Date: 18 May 2001
 * 
 * Copyright (C) 2000-2001, Romka Graphics This library is freely distributable without any license or permissions for
 * non-commercial usage. You can use this library in any non-commercial program. In each program, where you use this
 * library you should keep this header (author name and coordinates)! For commercial usage, please contact me:
 * romka@ut.ee
 * 
...
public class GLF {
	...
	/*
	 * | Function loads font to memory from file | Return value: GLF_ERROR - if error | >=0 - returned font descriptor
	 * (load success)
	 */
	...
	/*
	 * | Unload current font from memory | Return value: GLF_OK - if all OK | GLF_ERROR - if error
	 */
	...
	private void DrawString(GL gl, String s, DrawSymbol funct) {
			...
			for (i = 0; i < s.length(); i++) {
				if ((fonts[curfont].symbols[s.charAt(i)] == null) || (s.charAt(i) == ' ')) {
					...
					if (s.charAt(i + 1) == ' ') {
						...
						if (fonts[curfont].symbols[s.charAt(i + 1)] == null)
							continue;
		...
		for (i = 0; i < s.length(); i++) {
			if (s.charAt(i) != ' ')
				funct.drawCharacter(gl, s.charAt(i));
			if ((fonts[curfont].symbols[s.charAt(i)] == null) || (s.charAt(i) == ' ')) {
				switch (m_direction) {
				case GLF_LEFT:
	...
	public void glfDraw3DWiredSymbol(GL gl, char s) {
		...
		if ((curfont < 0) || (fonts[curfont] == null))
			return;
		if (fonts[curfont].symbols[(int) s] == null)
			return;
	...
	// }
	//
	// static void rgbatorgba(unsigned char *r,unsigned char *g,unsigned char *b,unsigned char *a,unsigned char *l,int
	// n)
	// {
	// while (n--) {
	// l[0] = r[0];
	...
	// static int[] read_texture(LittleEndianDataInputStream in, int width, int height, int components)
	// {
	// unsigned * base, *lptr;
